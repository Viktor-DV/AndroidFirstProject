...
class RatingDonutView @JvmOverloads constructor(
    ...
    private var progress = 50 // Начальное значение
    ...
    // Для получения текущего прогресса
    fun getProgress(): Int = progress

    // Для установки прогресса с возможностью анимации
    fun setProgress(newProgress: Int, animate: Boolean = false) {
        if (animate) {
            val animator = ValueAnimator.ofInt(progress, newProgress.coerceIn(0, 100))
            animator.duration = 1500 // 1.5 секунды
